...
function run()
{
    cd /home/liao/code/chat/GateServer/build
...
function start_server()
{
    ...
    if kill -0 $SERVER_PID 2>/dev/null; then
        ...
        if ss -tuln | grep ':8080' > /dev/null; then
            ...
            # 尝试发送测试请求
            if curl -s http://localhost:8080/get_test > /dev/null; then
...
function stop_server()
{
    if [ -f server.pid ]; then
        ...
        # 尝试查找可能的GateServer进程
        PIDS=$(pgrep GateServer)
        if [ ! -z "$PIDS" ]; then
            kill $PIDS
            echo "Killed GateServer process(es): $PIDS"
        fi
    fi
}
