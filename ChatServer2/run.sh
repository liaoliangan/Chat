...
function run()
{
    cd /home/liao/code/chat/ChatServer2/build
...
function start_server()
{
    ...
    if kill -0 $SERVER_PID 2>/dev/null; then
        ...
        if ss -tuln | grep ':8091' > /dev/null; then
            ...
            echo "! Port 8091 is not listening - server may have crashed"
        fi
    else
...
function stop_server()
{
    if [ -f server.pid ]; then
        ...
        # 尝试查找可能的ChatServer进程
        PIDS=$(pgrep ChatServer2)
        if [ ! -z "$PIDS" ]; then
            kill $PIDS
            echo "Killed ChatServer2 process(es): $PIDS"
        fi
    fi
}
