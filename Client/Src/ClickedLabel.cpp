...
void ClickedLabel::mouseReleaseEvent(QMouseEvent* event)
{
    if (event->button() == Qt::LeftButton)
    {
        if (_cur_state == LA::ClickLbState::Normal)
        {
            COUT << "clicked , change to normal press: " << _press.toStdString() << ENDL;
            setProperty("state", _hover);
            LA::repolish(this);
            update();
        }
        ...
        {
            COUT << "clicked , change to selected press: " << _selected_press.toStdString() << ENDL;
            setProperty("state", _selected_hover);
            LA::repolish(this);
            update();
        }
        emit clicked(this->text(), _cur_state);
        return;
    }
...
bool ClickedLabel::SetCurState(LA::ClickLbState state)
{
    _cur_state = state;
    if (_cur_state == LA::ClickLbState::Normal) {
        setProperty("state", _normal);
    ...
    else if (_cur_state == LA::ClickLbState::Selected) {
        setProperty("state", _selected);
        LA::repolish(this);
    }
